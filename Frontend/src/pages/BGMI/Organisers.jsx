...
const BGMIOrganisers = () => {
  const { data: teams, loading, error } = useFetch(`${BASE_URL}/organizers?game=bgmi`);
  const { user } = useAuth();
    ...
    <div className="container page-anim" style={{ paddingTop: '8rem' }}>
      ...
      <div className="grid-auto">
        {teams && teams.map(team => (
          <div key={team._id} className="glass-panel feature-card" style={{ padding: '1.5rem', borderTop: `4px solid ${team.color || '#f97316'}` }}>
            <div>
            ...
            <Link to={`/bgmi/organisers/${team.slug}`} className="btn btn-outline" style={{ width: '100%', textAlign: 'center', borderColor: team.color || '#f97316', color: team.color || '#f97316' }}>
              Know About More
            </Link>
          </div>
        ))}
