...
const THEME = '#ec4899';
const GLOW = 'rgba(236, 72, 153, 0.45)';

const FreeFireTournaments = () => {
  const { data: tournaments, loading, error } = useFetch(`${BASE_URL}/tournaments?game=freefire`);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out this FreeFire tournament: ${tournament.name}!`,
          url: url,
        });
      } catch (err) {
  ...
  return (
    <div className="container page-anim" style={{ paddingTop: '8rem' }}>
      <Link to="/freefire" className="go-back-link" style={{ color: THEME }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4rem' }}>
        {tournaments && tournaments.length > 0 ? (
          [...new Set(tournaments.map(t => t.organisingTeam))].map(teamName => {
            const teamTournaments = tournaments.filter(t => t.organisingTeam === teamName);
            return (
              <div key={teamName}>
                  ...
                  {teamTournaments.map(t => (
                    ...
                    >
                      ...
                      <div style={{ position: 'absolute', top: '1rem', right: '1rem' }}>
                        <span style={{
                          background: t.status === 'Upcoming' ? 'rgba(34,211,238,0.15)' : `rgba(236,72,153,0.2)`,
                      ...
                      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.75rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                          <Link
                            ...
                            style={{ '--card-accent': `linear-gradient(135deg, ${THEME}, #f97316)`, '--card-glow': GLOW, flex: 1 }}
                          >
                            Explore
                          </Link>
                          <button
